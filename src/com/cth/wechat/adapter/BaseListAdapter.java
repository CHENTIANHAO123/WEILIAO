...
public abstract class BaseListAdapter<E> extends BaseAdapter {
	...
	public interface onInternalClickListener {
		public void onClickListener(View parentView, View v, Integer position,
				Object values);
	}

	public Map<Integer, onInternalClickListener> canClickItem;

	private void addInternalClickListener(final View itemView,
				...
				if (inView != null && inviewListener != null) {
					inView.setOnClickListener(new OnClickListener() {
						...
						public void onClick(View v) {
							inviewListener.onClickListener(itemView, v,
									position, valuesMap);
						}
					});
				}
	...
	public void showToast(final String text) {
		if (!TextUtils.isEmpty(text)) {
			((Activity) mContext).runOnUiThread(new Runnable() {
				...
				public void run() {
					if (toast == null) {
						toast = Toast.makeText(mContext, text,
								Toast.LENGTH_SHORT);
					} else {
						toast.setText(text);
					}
